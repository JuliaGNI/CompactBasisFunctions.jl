...
@testset "$(rpad("Chebyshev Basis Tests",80))" begin
    ...
    @test [(d * u)[1.0, i] for i in eachindex(u)] ==
          [2 * i * (i+1) * (i+2)//3 for i in eachindex(u)]
    @test parent([(d * u)[1.0, i] for i in eachindex(u)]) == [0, 4, 16, 40, 80, 140]

    @test [(d * u)[0.0, i] for i in eachindex(u)] ==
          [2 * (-1)^(i+1) * i * (i+1) * (i+2)//3 for i in eachindex(u)]
    ...
    @test [(dt * t)[0.0, i] for i in eachindex(t)] ==
          [2 * (-1)^(i+1) * i^2 for i in eachindex(t)]
    ...
    # rather than a benchmark, cf. the same guard in bernstein.jl.
    for kind in (1, 2)
        b = Chebyshev{kind}(80)
